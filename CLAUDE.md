# Claude Code Project Instructions — assert-matches-kotlin

## Project Overview

This is **assert-matches-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`assert_matches`](https://crates.io/crates/assert_matches).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/assert_matches/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/assert_matches>
