...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`assert_matches`](https://crates.io/crates/assert_matches).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `assert_matches` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

`assert_matches!` macro for pattern assertions
...
Original work copyrighted by the upstream `assert_matches` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
