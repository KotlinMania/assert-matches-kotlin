...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `AssertMatches.swiftmodule/`
...
//      `import AssertMatches` above would fail at compile time.
//
//   2. The static archive `libAssertMatches.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "AssertMatches" }` and
//      `flattenPackage = "io.github.kotlinmania.assertmatches"` configuration in
...
//      via the `AssertMatchesLibrary` product.
//
...
final class AssertMatchesExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "AssertMatches swift module imported cleanly")
    }
}
